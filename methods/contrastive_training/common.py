...
from utils.cluster_utils import my_mixed_eval, cluster_acc
...
class SupConLossWithMembank(torch.nn.Module):
    ...
    def forward(self, features, labels=None, mask=None, mfeatures=None, mlabels=None):
        ...
        if mfeatures is not None:
            ...
            assert (mlabels is not None) and (mlabels.shape == torch.Size(
                [mfeatures.size(0)])), f'mlabels={mlabels.shape if mlabels is not None else None}'
        ### <<<
...
### =====================================================================================================
def compute_cross_contrastive_loss_sup(q_features, q_labels, q_mask, k_features, k_labels, k_mask, sup_cont_crit=None,
                                       n_views=2):
    ...
    sup_contrastive_loss = sup_cont_crit(features=q_sup_con_feats, labels=q_sup_con_labels, mask=None,
                                         mfeatures=k_sup_con_feats, mlabels=k_sup_con_labels)
...
def forward(x, model, projection_head=None, predict_token='cls',
    ...
    if predict_token == 'cls':
        ...
        features = [features[:, 0, :],
                    features[:, 1:1 + num_cop, :].view(shape_feature[0], -1, shape_feature[-1]).mean(dim=1)]
...
def test_kmeans(model, test_loader,
    ...
    if use_fast_Kmeans is True:
        ...
        kmeans = K_Means(k=args.num_labeled_classes + args.num_unlabeled_classes, tolerance=1e-6, max_iterations=500,
                         ...
                         pairwise_batch_size=None if all_feats.size(
                             0) < args.fast_kmeans_batch_size else args.fast_kmeans_batch_size,
                         mode=None)
        ...
        kmeans = KMeans(n_clusters=args.num_labeled_classes + args.num_unlabeled_classes, random_state=0,
                        verbose=0).fit(all_feats)
        end = time.time()
        print(f'time={end - begin}')
...
def test_kmeans_val(model, test_loader,
    ...
    if stage == 1:  ### only consider lbl data
        if use_fast_Kmeans is True:
            begin = time.time()
            all_feats = all_feats.to(args.device)
            kmeans = K_Means(k=args.num_labeled_classes + args.num_unlabeled_classes, tolerance=1e-6,
                             max_iterations=500, init='k-means++',
                             n_init=20, random_state=0, n_jobs=1,
                             pairwise_batch_size=None if all_feats.size(
                                 0) < args.fast_kmeans_batch_size else args.fast_kmeans_batch_size,
                             mode=None)
...
def compute_knn_statistics_with_affinity(query_label, mb_labels, knn_affinity, eps=1e-10):
    ...
    precision = (knn_match[knn_affinity == 1].float() + 1e-10).mean() if (
                                                                                     knn_affinity == 1).sum().item() != 0 else torch.tensor(
        0.0).to(knn_affinity.device)

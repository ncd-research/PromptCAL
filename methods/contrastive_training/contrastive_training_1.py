...
from utils.config import exp_root, dino_pretrain_path, ibot_pretrain_path
...
def train(projection_head, model, train_loader, test_loader, unlabelled_train_loader, args, aux_dataloader=None,
          aux_model=None, val_loader=None):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                if epoch < args.inkd_T:
                    try:
                        ...
                        aux_images, _ = next(iter_aux_dataloader)
                        aux_images = aux_images.to(args.device)
                    inkd_loss = forward_single_inkd(model, aux_model, aux_images, loss_func=F.mse_loss,
                                                    distill='features')
                    loss += max(0,
                                annealing_decay(args.w_inkd_loss, args.w_inkd_loss_min, epoch, args.inkd_T)) * inkd_loss

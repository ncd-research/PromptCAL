...
python -m methods.clustering.extract_features \
  --dataset scars \
  --warmup_model_dir '/data01/yuho_hdd/refactored_gcd/cache/promptcal/stage2/scars/model.pt' \
...
python -m methods.clustering.extract_features \
  --dataset cifar100 \
  --warmup_model_dir '/data01/yuho_hdd/refactored_gcd/cache/promptcal/stage2/cifar100/model.pt' \
  --model_name 'vpt-model' \
  --transform 'imagenet' \
  --num_prompts 5 \
  --device "cuda:${GPU}" \
...
python -m methods.clustering.extract_features \
  --dataset cub \
  --warmup_model_dir '/data01/yuho_hdd/refactored_gcd/cache/promptcal/stage2/cub/model.pt' \
  --model_name 'vpt-model' \
  --transform 'imagenet' \
  --num_prompts 5 \
  --device "cuda:${GPU}" \

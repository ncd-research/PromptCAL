...
SAVE_DIR=/data01/yuho_hdd/PromptCAL/cache/$EXP_NAME/
...
python -m methods.clustering.extract_features \
  --dataset ${DATASET} \
  --use_best_model ${postfix_2} \
  --warmup_model_dir ${MODEL_PATH} \
  --model_name ${MODEL_NAME} \
  --transform ${TRANSFORM} \
  --num_prompts ${NUM_PROMPTS} \
  --device ${DEVICE} \
  --n_shallow_prompts ${N_SHALLOW_PROMPTS} \
